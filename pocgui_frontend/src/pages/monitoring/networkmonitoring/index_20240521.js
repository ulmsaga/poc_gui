...
import { getEnbTreeList, getMmeTreeList } from "api/nw/configApi";
import Expandable from "components/virtualized-tree/renderers/Expandable";
...
const NetworkMonitoring = () => {
  ...
  // const [mmeTree, setMmeTree] = useState([]);
  ...
  const TreeComp = ({ style, node, ...rest }) => {
    return (
      <div style={ style }>
  ...
  useEffect(() => {
    ...
    getMmeTreeList({}).then((response) => { console.log('getMmeTreeList', setState({nodes: response.data.rs})); });
    getEnbTreeList({}).then((response) => { console.log('getEnbTreeList', response.data.rs); });
