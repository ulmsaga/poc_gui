...
import bgLogin01 from 'assets/images/login/bg_login_02.png';
...
const LoginBackground = () => {
  ...
  return (
    <Fragment>
      <div style={{
        backgroundImage: `url(${bgLogin01})`,
        position: 'absolute',
        width: 'calc(100vw - 10px)', // 화면 너비에 맞춤
        height: 'calc(100vh - 10px)', // 화면 높이에 맞춤
        backgroundSize: 'cover', // 이미지가 화면을 꽉 채우도록 조정
        backgroundPosition: 'center', // 이미지가 중앙에 위치하도록 조정
        filter: 'blur(8px)',
        zIndex: -1 // 배경이 되도록 z-index 조정
      }} />
      <Box sx={{ position: 'absolute', filter: 'blur(14px)', zIndex: -1, bottom: 0 }}>
